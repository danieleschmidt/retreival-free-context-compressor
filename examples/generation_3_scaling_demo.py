...
async def demo_basic_adaptive_compression():
    ...
    try:
        ...
        for i, text in enumerate(SAMPLE_TEXTS[:3]):
            ...
            print(
                f"  Strategy Used: {result.metadata.get('selected_strategy', 'unknown')}"
            )
        ...
        if performance:
            ...
            for strategy_key, stats in performance.items():
                print(
                    f"  {strategy_key}: {stats['avg_compression_ratio']:.1f}x avg ratio, {stats['avg_processing_time']:.2f}s avg time"
                )
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            for device in stats.get("devices", []):
                print(
                    f"  - {device['name']} ({device['device_type']}): {device['memory_total_mb']:.0f}MB total"
                )
        ...
        if len(SAMPLE_TEXTS) > 1:
            ...
            print(
                f"  Total Compression Ratio: {sum(r.compression_ratio for r in results)/len(results):.1f}x"
            )
...
async def demo_async_processing():
    ...
    try:
        ...
        print(
            f"  Average Compression Ratio: {sum(r.compression_ratio for r in results)/len(results):.1f}x"
        )
...
async def demo_performance_monitoring():
    ...
    try:
        ...
        monitor = get_performance_monitor(
            collection_interval=1.0, analysis_interval=5.0  # Fast collection for demo
        )
        await monitor.start()
        ...
        if report["metrics"]:
            ...
            for metric_name, stats in report["metrics"].items():
                if "mean" in stats:
                    print(
                        f"  {metric_name}: {stats['mean']:.2f} (avg), {stats['max']:.2f} (max)"
                    )
...
async def demo_streaming_compression():
    ...
    try:
        from retrieval_free.adaptive_compression import (
            StreamingCompressor,
            AdaptiveCompressor,
        )
        ...
        for i, text in enumerate(SAMPLE_TEXTS):
            ...
            if compressed_tokens:
                print(
                    f"  Stream {i+1}: Compressed to {len(compressed_tokens)} mega-tokens"
                )
            else:
            ...
            print(
                f"    Buffer: {stats['buffer_size']}, Processed: {stats['processed_count']}, Ratio: {stats['compression_ratio']:.1f}x"
            )
...
async def demo_full_pipeline():
    ...
    try:
        ...
        print(
            f"  Single Text: {single_result.compression_ratio:.1f}x in {single_time:.2f}s"
        )
        ...
        print(
            f"  Batch ({len(SAMPLE_TEXTS)} texts): avg {sum(r.compression_ratio for r in batch_results)/len(batch_results):.1f}x in {batch_time:.2f}s"
        )
        ...
        print(
            f"  Async Text: {async_result.compression_ratio:.1f}x in {async_time:.2f}s"
        )
        ...
        print(
            f"  Operations Completed: 1 single + 1 batch ({len(SAMPLE_TEXTS)}) + 1 async"
        )
