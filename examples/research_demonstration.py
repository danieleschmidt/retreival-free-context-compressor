...
async def demo_novel_compression_objectives():
    ...
    try:
        ...
        for obj_name, objective in objectives:
            ...
            print(
                f"  Quality Score: {result.metadata.get('reconstructed_quality', 0):.3f}"
            )
            print(f"  Mega-tokens: {len(result.mega_tokens)}")

            if (
                hasattr(objective, "performance_history")
                and objective.performance_history
            ):
                print(
                    f"  Adaptive History: {len(objective.performance_history)} updates"
                )
...
async def demo_comparative_analysis():
    ...
    try:
        ...
        analyzer.add_baseline(
            "Baseline_RAG", baseline_compressor
        )  # Using same for demo
        ...
        for method_name, results in comparative_results.items():
            ...
            avg_quality = sum(r.metrics.semantic_fidelity for r in results) / len(
                results
            )
        ...
        if "statistical_tests" in statistical_report:
            ...
            for test_name, test_result in statistical_report[
                "statistical_tests"
            ].items():
                significance = (
                    "✓ Significant"
...
async def demo_scaling_law_analysis():
    ...
    try:
        ...
        print(
            f"🔍 Analyzing scaling across {len(text_lengths)} text lengths and {len(compression_ratios)} ratios..."
        )
        ...
        if "predictions" in scaling_results:
            ...
            if "compression_ratios" in predictions:
                ...
                print(
                    f"  Compression at 10k tokens: {comp_pred['predicted_ratios'][0]:.1f}x"
                )
            ...
            if "processing_times" in predictions:
                ...
                print(
                    f"  Time at 10k tokens: {time_pred['predicted_times_sec'][0]:.3f}s"
                )
        ...
        if "optimal_parameters" in scaling_results:
            ...
            if "best_efficiency" in optimal:
                ...
                print(
                    f"  Best Efficiency: {best_eff['compression_ratio']:.1f}x @ {best_eff['text_length']} chars"
                )
...
async def demo_publication_ready_experiment():
    ...
    try:
        ...
        if "statistical_analysis" in results:
            ...
            if "performance_analysis" in stats:
                ...
                for method, analysis in stats["performance_analysis"].items():
                    ...
                    print(
                        f"    {method}: {comp_ratio:.1f}x ratio, {proc_time:.3f}s time"
                    )
        ...
        if "reproducibility" in experiment_results:
            ...
            print(
                f"  Dependencies: {len(repro['dependencies']['required_packages'])} packages"
            )
...
async def demo_advanced_research_metrics():
    ...
    try:
        ...
        print(
            f"  Std Compression: {(sum((r - sum(all_ratios)/len(all_ratios))**2 for r in all_ratios) / len(all_ratios))**0.5:.2f}"
        )
        print(f"  Mean Fidelity: {sum(all_fidelities)/len(all_fidelities):.3f}")
        print(
            f"  Content Type Variance: {len(set(exp.dataset_name for exp in experiment_results))} types tested"
        )

        # Research validity assessment
        high_quality_results = [
            exp for exp in experiment_results if exp.metrics.semantic_fidelity > 0.8
        ]

        print(f"\n✅ Research Validity Assessment:")
        print(
            f"  High-quality results: {len(high_quality_results)}/{len(experiment_results)}"
        )
        print(f"  Reproducibility: All results include hash signatures")
        print(
            f"  Statistical rigor: Confidence intervals and significance tests included"
        )
