...
from .compliance import LegalBasis, ProcessingPurpose, get_compliance_manager
...
from .core import CompressionResult, ContextCompressor
...
class EnterpriseCompressionResponse:
    ...
    compliance_record: str | None = None
...
class EnterpriseContextCompressor:
    ...
    def _setup_compressor(self) -> None:
        ...
        model_name = self.config_manager.get(
            "compression.model_name", "sentence-transformers/all-MiniLM-L6-v2"
        )
        chunk_size = self.config_manager.get("compression.chunk_size", 512)
        compression_ratio = self.config_manager.get(
            "compression.compression_ratio", 8.0
        )
        ...
        if not security_scan.passed:
            logger.warning(
                f"Model security scan failed: {security_scan.vulnerabilities}"
            )
        ...
        self.circuit_breaker = get_circuit_breaker(
            "compression", circuit_breaker_config
        )
    ...
    def _setup_enterprise_features(self) -> None:
        ...
        self.feature_manager.register_toggle(
            "pii_detection", FeatureToggleStrategy.ON, enabled=True
        )
        ...
        self.feature_manager.register_toggle(
            "compliance_logging", FeatureToggleStrategy.ON, enabled=True
        )
    ...
    def _setup_validation_schemas(self) -> None:
        ...
        request_rules = [
            ...
            ValidationRule(
                "user_id", "type", str, "User ID must be a string", "warning"
            ),
            ValidationRule(
                "compression_ratio",
                "range",
        ...
        response_rules = [
            ...
            ValidationRule(
                "trace_id", "type", str, "Trace ID must be string", "warning"
            ),
        ]
    ...
    @with_resilience(
        "enterprise_compression", timeout_seconds=30, enable_graceful_degradation=True
    )
    def compress(
        self, request: EnterpriseCompressionRequest, **kwargs
        ...
        try:
            ...
            if self.feature_manager.is_enabled("pii_detection", user_id):
                sanitization_result = self.input_sanitizer.sanitize_input(
                    request.text, mask_pii=request.enable_pii_masking
                )

                if sanitization_result["pii_detected"]:
                    warnings.append(
                        f"PII detected: {sanitization_result['pii_detected']}"
                    )
                ...
                security_scan = {
                    ...
                    "malicious_patterns": sanitization_result[
                        "malicious_patterns_found"
                    ],
                }

                # Update metrics
                pii_detection_rate = len(sanitization_result["pii_detected"]) / max(
                    len(request.text), 1
                )
                ...
                if user_id:
                    ...
                    if user_id not in self.compliance_manager.data_subjects:
                        self.compliance_manager.register_data_subject(
                            user_id, consent=True  # In production, get real consent
                        )
            ...
            compression_params = {"text": sanitized_text}
            ...
            performance_metrics = {
                ...
                "memory_usage_mb": self.resource_manager.check_memory_usage().get(
                    "memory_mb", 0
                ),
            }
            ...
            if memory_status.get("cleanup_recommended", False):
                ...
                warnings.append(
                    f"Memory cleanup performed: saved {cleanup_summary.get('memory_saved_mb', 0):.1f}MB"
                )
            ...
            # Log error
            self.audit_logger.log_security_event(
                "compression_error", f"Compression failed: {str(e)}", "error"
            )
    ...
    def run_enterprise_health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            key_id, api_key = self.auth_manager.generate_api_key(
                permissions={"compress", "health"}, rate_limit_requests=1000
            )
            ...
            health_results["components"]["compression"] = {
                ...
                "details": {
                    "compressed_tokens": (
                        len(response.result.mega_tokens) if response.result else 0
                    ),
...
def create_enterprise_compressor(
    config_dir: str | None = None,
) -> EnterpriseContextCompressor:
