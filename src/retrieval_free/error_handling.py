...
from .exceptions import (
    RetrievalFreeError, CompressionError, ModelError, ResourceError
)
...
class RetryConfig:
    ...
    retryable_exceptions: Tuple[type, ...] = (
        ConnectionError, TimeoutError, ResourceError
    )
        ...
        with self._lock:
            ...
            try:
                ...
                raise CompressionError(
                    ...
                    details={'circuit_breaker': self.name, 'original_exception': str(e)}
                )
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                result = func(*args, **kwargs)
                if attempt > 0:
                    logger.info(f"Retry mechanism '{self.name}' succeeded on attempt {attempt + 1}")
                return result
...
class TimeoutHandler:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self._start_time:
            ...
            if elapsed > self.timeout:
                logger.warning(f"Operation '{self.name}' exceeded timeout: {elapsed:.2f}s > {self.timeout}s")
    ...
    def check_timeout(self) -> None:
        ...
        elapsed = time.time() - self._start_time
        if elapsed > self.timeout:
            raise TimeoutError(f"Operation '{self.name}' timed out after {elapsed:.2f}s")
...
class ResourceManager:
    ...
    def check_memory_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            usage_info = {
                ...
                'gpu_memory_usage_percent': (gpu_memory_mb / self._gpu_memory_threshold_mb) * 100 if self._gpu_memory_threshold_mb > 0 else 0,
                'tracked_resources': len(self._tracked_resources)
            }
            
            # Check if cleanup is needed
            if (memory_mb > self._memory_threshold_mb or 
                gpu_memory_mb > self._gpu_memory_threshold_mb):
                logger.warning(f"High memory usage detected: RAM {memory_mb:.1f}MB, GPU {gpu_memory_mb:.1f}MB")
    ...
    def cleanup_resources(self, force: bool = False) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            try:
                ...
                cleanup_summary['gpu_memory_before_mb'] = initial_usage.get('gpu_memory_mb', 0)
                ...
                cleanup_summary['gpu_memory_after_mb'] = final_usage.get('gpu_memory_mb', 0)
                cleanup_summary['resources_after'] = len(self._tracked_resources)
                
                # Calculate savings
                memory_saved = cleanup_summary['memory_before_mb'] - cleanup_summary['memory_after_mb']
                gpu_memory_saved = cleanup_summary['gpu_memory_before_mb'] - cleanup_summary['gpu_memory_after_mb']
    ...
    def _cleanup_dead_references(self) -> None:
        ...
        self._tracked_resources = [ref for ref in self._tracked_resources if ref() is not None]
        ...
        for func_type, func in functions_to_try:
            try:
                ...
                if func_type != 'primary':
                    logger.warning(f"Graceful degradation '{self.name}' used {func_type}")
                return result
...
def get_retry_mechanism(name: str, config: Optional[RetryConfig] = None) -> RetryMechanism:
