...
class MemoryOptimizer:
    ...
    def force_cleanup(self) -> dict[str, float]:
        ...
        stats = {
            ...
            "freed_mb": freed_mb,
            "freed_percent": (
                (freed_mb / memory_before * 100) if memory_before > 0 else 0
            ),
        }

        if freed_mb > 1.0:  # Only log if significant cleanup
            logger.info(
                f"Memory cleanup freed {freed_mb:.1f}MB ({stats['freed_percent']:.1f}%)"
            )
    ...
    def memory_efficient_context(self, clear_cache: bool = True):
        ...
        try:
            ...
            if memory_delta > 100:  # Log significant memory increases
                logger.warning(
                    f"Operation increased memory usage by {memory_delta:.1f}MB"
                )
...
class BatchProcessor:
    ...
    def process_in_batches(
        ...
        for i in range(0, len(items), batch_size):
            ...
            if show_progress:
                logger.info(
                    f"Processing batch {batch_idx}/{total_batches} ({len(batch)} items)"
                )
        ...
        if len(items) > 0:
            ...
            if memory_per_item > 0:
                optimal_batch_size = min(
                    batch_size, max(1, int(max_memory_mb / memory_per_item))
                )
            else:
                optimal_batch_size = batch_size

            logger.info(
                f"Adaptive batching: {memory_per_item:.1f}MB/item, using batch_size={optimal_batch_size}"
            )
            ...
            remaining_results = self.process_in_batches(
                remaining_items, process_func, optimal_batch_size, show_progress=True
            )
...
class ModelOptimizer:
    ...
    def optimize_embedding_computation(
        self, embeddings: "torch.Tensor", target_dtype: Optional["torch.dtype"] = None
    ) -> "torch.Tensor":
