...
class SchemaValidator:
    ...
    def _setup_default_schemas(self) -> None:
        ...
        compression_input_rules = [
            ...
            ValidationRule(
                "model_name", "type", str, "Model name must be a string", "warning"
            ),
            ValidationRule(
                "compression_ratio",
                "range",
                (1.0, 100.0),
                "Compression ratio must be between 1 and 100",
            ),
            ValidationRule(
                "chunk_size",
                "range",
                (32, 8192),
                "Chunk size must be between 32 and 8192",
            ),
        ]
        ...
        api_response_rules = [
            ...
            ValidationRule(
                "data", "required", None, "Data field is required for success responses"
            ),
            ValidationRule(
                "error", "type", dict, "Error must be a dictionary", "warning"
            ),
        ]
        self.schemas["api_response"] = api_response_rules

        # Custom validators
        self.custom_validators["non_empty_text"] = (
            lambda x: isinstance(x, str) and x.strip() != ""
        )
        self.custom_validators["valid_model_name"] = (
        ...
        try:
            ...
            if rule.rule_type == "required":
                ...
                if field_value is not None and not isinstance(
                    field_value, rule.constraint
                ):
                    self._add_error(
                        result, rule, f"Field must be of type {rule.constraint}"
                    )
                    return False
                ...
                if field_value is not None:
                    ...
                    if not re.match(rule.constraint, str(field_value)):
                        self._add_error(
                            result,
                            rule,
                            f"Value does not match pattern {rule.constraint}",
                        )
                        return False
                    ...
                    if validator_name in self.custom_validators:
                        ...
                        if not validator(field_value):
                            self._add_error(
                                result,
                                rule,
                                f"Custom validation failed: {validator_name}",
                            )
                            return False
                    else:
                        self._add_error(
                            result, rule, f"Unknown custom validator: {validator_name}"
                        )
                        return False
    ...
    def _add_error(
        self, result: ValidationResult, rule: ValidationRule, message: str
    ) -> None:
...
class PerformanceProfiler:
    ...
    def _process_profile_results(
        ...
        if initial_memory and final_memory:
            for key in initial_memory:
                if key in final_memory and isinstance(
                    initial_memory[key], (int, float)
                ):
                    memory_delta[f"{key}_delta"] = (
                        final_memory[key] - initial_memory[key]
                    )
        ...
        with self._lock:
            ...
            if len(self.profiles) > 100:
                oldest_id = min(
                    self.profiles.keys(), key=lambda x: self.profiles[x]["timestamp"]
                )
    ...
    def get_profile_summary(self, operation_name: str | None = None) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if operation_name:
                profiles = [
                    p for p in profiles if p["operation_name"] == operation_name
                ]
...
class TestFramework:
    ...
    def register_test_suite(self, suite_name: str, test_cases: list[TestCase]) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Registered test suite '{suite_name}' with {len(test_cases)} test cases"
            )
    ...
    def run_test_suite(
        ...
        if parallel and len(test_cases) > 1:
            with concurrent.futures.ThreadPoolExecutor(
                ...
                future_to_test = {
                    executor.submit(
                        self._run_single_test, test_case, target_function
                    ): test_case
        ...
        logger.info(
            f"Test suite '{suite_name}' completed: {successful_tests}/{len(results)} passed"
        )

        return results

    def _run_single_test(
        self, test_case: TestCase, target_function: Callable
    ) -> TestResult:
        ...
        with tracer.span(f"test_{test_case.test_id}") as span:
            ...
            try:
                ...
                # Check timeout
                if (
                    test_case.max_duration_ms
                    and duration_ms > test_case.max_duration_ms
                ):
                    return TestResult(
                        test_id=test_case.test_id,
                        success=False,
                ...
                for criterion in test_case.success_criteria:
                    try:
                        if not criterion(output):
                            success = False
                            error_msg = (
                                f"Success criterion failed: {criterion.__name__}"
                            )
                            break
                    except Exception as e:
                        success = False
                ...
                # Record metrics
                metrics = {"duration_ms": duration_ms, "success": 1 if success else 0}
    ...
    def run_load_test(
        ...
        """
        logger.info(
            f"Starting load test: {config.concurrent_users} users, {config.duration_seconds}s"
        )
        ...
        def worker():
            ...
            while time.time() < end_time:
                try:
                    test_data = (
                        random.choice(config.test_data) if config.test_data else {}
                    )
        ...
        requests_per_second = (
            total_requests / config.duration_seconds
        ...
        logger.info(
            f"Load test completed: {successful_requests}/{total_requests} successful ({requests_per_second:.1f} RPS)"
        )
    ...
    def run_chaos_test(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            concurrent.futures.wait(
                [chaos_future, operations_future], timeout=test_duration_seconds + 30
            )
        ...
        logger.info(
            f"Chaos test completed: {len(chaos_events)} chaos events, {successful_operations}/{total_operations} operations succeeded"
        )
    ...
    def get_test_summary(self, test_type: str | None = None) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "test_types": list(
                    set(r.test_id.split("_")[0] for r in results if "_" in r.test_id)
                ),
                "recent_failures": [
        ...
        def wrapper(*args, **kwargs):
            ...
            if isinstance(result, dict):
                ...
                if not output_result.valid:
                    logger.warning(
                        f"Output validation failed: {'; '.join(output_result.errors)}"
                    )
