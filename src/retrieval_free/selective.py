...
class ImportanceAnalyzer:
    ...
    def analyze_segments(
        self, 
        segments: List[str],
        segment_positions: Optional[List[Tuple[int, int]]] = None
    ) -> List[ContentSegment]:
    ...
    def _calculate_importance_score(self, text: str, features: Dict[str, Any]) -> float:
        ...
        if features['sentence_count'] > 0:
            density_score = min(1.0, (features['word_count'] / features['sentence_count']) / 20)
        else:
            density_score = 0.0
...
class SelectiveCompressor(CompressorBase):
    ...
    def compress(
        ...
        custom_importance_fn: Optional[Callable[[str], float]] = None,
        **kwargs
    ) -> CompressionResult:
        ...
        try:
            ...
            with self._memory_optimizer.memory_efficient_context():
                ...
                if custom_importance_fn:
                    for segment in content_segments:
                        ...
                        segment.importance_score = max(segment.importance_score, custom_score)
                        segment.importance_level = self._importance_analyzer._score_to_level(
                ...
                for segment in content_segments:
                    ...
                    # Update statistics
                    self._compression_stats['importance_distribution'][segment.importance_level.value] += 1
                
                # Update statistics
                self._compression_stats['segments_processed'] += len(content_segments)
                self._compression_stats['compression_ratios_used'].extend(compression_ratios_used)
            ...
            overall_ratio = original_length / compressed_length if compressed_length > 0 else 1.0
            ...
            result = CompressionResult(
                ...
                metadata={
                    ...
                    'importance_distribution': {
                        level.value: sum(1 for s in content_segments if s.importance_level == level)
                        for level in ImportanceLevel
                    },
                    'adaptive_ratios_used': self.adaptive_ratios,
                    'compression_ratios_range': {
                        'min': min(compression_ratios_used) if compression_ratios_used else self.compression_ratio,
                        'max': max(compression_ratios_used) if compression_ratios_used else self.compression_ratio,
                        'avg': np.mean(compression_ratios_used) if compression_ratios_used else self.compression_ratio
                    }
                }
            )
    ...
    def decompress(
        ...
        **kwargs
        ...
        for i, token in enumerate(mega_tokens):
            ...
            compression_ratio = token.metadata.get('target_compression_ratio', token.compression_ratio)
    ...
    def get_compression_strategy(self, text: str) -> Dict[str, Any]:
        ...
        if not validation_result.is_valid:
            raise ValidationError("Input validation failed", validation_errors=validation_result.errors)
        
        text = validation_result.sanitized_input['text']
        
        # Segment and analyze
        segments, positions = self._segment_text(text)
        content_segments = self._importance_analyzer.analyze_segments(segments, positions)
        ...
        for segment in content_segments:
            segment_info = {
                ...
                'importance_score': segment.importance_score,
                'importance_level': segment.importance_level.value,
                'target_ratio': segment.get_target_compression_ratio() if self.adaptive_ratios else self.compression_ratio,
        ...
        strategy['expected_compression_ratio'] = len(text) / max(1, strategy['expected_output_size'])
